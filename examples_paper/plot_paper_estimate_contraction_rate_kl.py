...
os.makedirs(os.path.join(path, 'ratekl'), exist_ok=True)
...
# FIXME ratekl?
rho_scale = 10 ** np.load(os.path.join(path, 'ratekl', f"rho_scale.npy"))

for r in range(1, len(eps_l)):
    epst = 10 ** eps_l[r]
    rate_s = np.load(os.path.join(path, 'ratekl', f"rate_f_sinkhorn_kl_eps{epst}.npy"))
    rate_ti = np.load(os.path.join(path, 'ratekl', f"rate_g_sinkhorn_kl_eps{epst}.npy"))
    rate_f = np.load(os.path.join(path, 'ratekl', f"rate_h_sinkhorn_kl_eps{epst}.npy"))
...
rho_scale = 10 ** np.load(os.path.join(path, 'rateberg', f"rho_scale.npy"))
for r in range(1, len(eps_l)):
    epst = 10 ** eps_l[r]
    rate_f = np.load(os.path.join(path, 'rateberg', f"rate_f_sinkhorn_berg_eps{epst}.npy"))
    rate_g = np.load(os.path.join(path, 'rateberg', f"rate_g_sinkhorn_berg_eps{epst}.npy"))
...
plt.tight_layout()
plt.savefig(os.path.join(path, 'paper', 'plot_log_contraction_rate_kl_berg_v2.pdf'))
